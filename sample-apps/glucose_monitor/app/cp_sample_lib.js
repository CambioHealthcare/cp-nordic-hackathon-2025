...
function addGrafanaChart(documentId, orgId, chartId, panelId) {
    const documentSelected = document.querySelector("#" + documentId);
    documentSelected.src = getDashboardUrl(chartId, ehrId, orgId, panelId);
}
...
function getDashboardUrl(dashboardId, ehrId, orgId = 1, panelId = 1) {
    return grafanaBaseUrl + "/d-solo/" + dashboardId + "?orgId=" + orgId + "&panelId=" + panelId + "&theme=light&query-param-ehr_id=" + ehrId + jwtTokenParam();
}
